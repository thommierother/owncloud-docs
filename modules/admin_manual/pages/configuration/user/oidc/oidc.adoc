...
+
To make the endpoint available under the static service discovery path, it is recommended to put a `RewriteRule` in place using `.htaccess` (the Apache modules `proxy` and `proxy_http` and `mod_rewrite` have to be enabled):
+
[source,apache]
----
...
----
+
TIP: Depending on the respective infrastructure setup there can be other ways to solve this. In any case, please make sure _not to use redirect rules_ (Apache "Redirect" from mod_alias instead of "Rewrite"). This will violate the OpenID Connect implementation of ownCloud.
+
TIP: If you use a load balancer or SSL accelerator in front of the webserver, it may be necessary to perform the rewrite there instead of the webserver (e.g. for HAPROXY, use `http-request set-path`s in the frontend configuration)
+
IMPORTANT: If you use the `.htaccatess` file in the ownCloud web root, you have to manually add that rewrite rule again after any ownCloud upgrade.
